...
const Project = {
  recommend: n => request.get("/project/recommend", { params: { n } }),
  hot: () => request.get("/project/hot"),
  byCategory: (
    categoryId,
    page = 0,
    size = 12,
    ...
    request.get("/project/byCategory", {
      params: { categoryId, page, size, direction, field }
    }),
  detail: id => request.get("/project/", { params: { id } }),
  byName: (
    name,
    page = 0,
    size = 12,
    ...
    request.get("/project/byName", {
      params: { name, page, size, direction, field }
    }),
  favoritedBy: userId =>
    request.get("/project/favoritedBy", { params: { userId } }),
  releasedBy: userId =>
    request.get("/project/releasedBy", { params: { userId } }),
  save: project => request.post("/project/", project),
  addSchedule: data => request.post("/project/schedule", data)
};
...
const Comment = {
  byProjectId: (
    ...
    request.get("/comment/byProjectId", {
      params: { projectId, page, size, direction, field }
    }),
...
const Auth = {
  login: loginForm => request.post("/auth/login", loginForm),
  sign: signForm => request.post("/user/sign", signForm)
};
...
const Banner = {
  all: () => request.get("/banner/all")
};

const Search = {
  project: form => request.get("/search/project", { params: form }),
  user: form => request.get("/search/user", { params: form }),
  news: form => request.get("/search/news", { params: form })
};
